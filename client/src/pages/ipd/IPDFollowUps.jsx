...
export default function IPDFollowUps() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setPatients(
          data.map((p) => ({ ...p, followUpDate: toDateStr(p.followUpDate) })),
        );
      } catch (err) {
  ...
  const updateFollowUpStatus = async (patientId, status) => {
    ...
    try {
      ...
      setPatients((ps) =>
        ps.map((p) =>
          p.id === patientId
            ? { ...updated, followUpDate: toDateStr(updated.followUpDate) }
            : p,
        ),
      );
    } catch (err) {
    ...
    return (
      ...
      >
        <div className="shrink-0">
          <div className="w-10 h-10 rounded-2xl bg-[#0f4a29]/10 text-[#0f4a29] dark:text-[#52b788] flex items-center justify-center font-extrabold text-sm">
            {p.name[0]}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 font-sans text-slate-900 bg-[#f4f5f7] dark:bg-slate-950 p-2 sm:p-4 rounded-3xl">
      <PageHeader
        title="IPD Follow-Up Management"
        subtitle="Scheduled inpatient follow-ups & automated WhatsApp reminders"
        action={
